...
#===================data with N observation row summarizing cooccurring observations=========================
...
#=========================================data with row for each observations=============================

allObsData <- read.csv("Data/stygian_cleaned_all.csv")
...
sunsetTimes <- getSunlightTimes(date=as.Date(allObsData$Date),lat=35.929086, lon=-82.881060,
                                keep="sunset", tz="America/New_York")$sunset %>%
  as.POSIXct(origin="1970-01-01", tz="America/New_York")
...
allObsData$Time <- as.POSIXct(allObsData$Time, origin="1970-01-01")
allObsData$Date <- as.POSIXct(allObsData$Date, origin="1970-01-01")

minTime <- min(allObsData$Time) %>%
  as.POSIXct()
maxTime <- max(allObsData$Time) %>%
  as.POSIXct()
...
ggplot() +
  geom_hex(allObsData, mapping=aes(x=Date, y=Time), bins=16) +
  #geom_point(obsData, pch=21, size=2, mapping = aes(x=Date, y=Time, fill=nObservations)) +
  #geom_line(obsData, mapping=aes(x=Date, y=sunsetTimes, color="Sunset Time"),size=1) +
  scale_x_datetime(breaks=waiver, date_breaks="5 days",labels = date_format("%b-%d")) +
  scale_y_datetime(breaks=as.POSIXct(numBreaks, origin="1970-01-01"),
